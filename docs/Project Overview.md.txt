...
- 8\. Project Governance and Version Control
- 9\. Project History and Revision Log
...
A new worksheet named Expected_Commissions will be created to record all orders and expected commissions. This allows PSI and VAs to capture anticipated commission amounts, which will later be reconciled against actual carrier payments to identify missed or short-paid commissions.

Key fields on the Expected_Commissions tab include: Carrier, Customer, AccountNo, Product, StartDate, ExpectedRate, ExpectedMRR, ExpectedPayType, Notes, EnteredBy, and EnteredDate. This tab becomes the foundation for the Expected vs. Actual comparison reports.

## 8\. Project Governance and Version Control

This document and all related code (M scripts, VBA macros, workbook versions) will be stored in a shared version-controlled repository such as OneDrive or GitHub. Each update will be saved as a new version (e.g., v2025.10.3) to preserve historical context. This ensures no loss of memory as the project evolves and allows both parties to reference the authoritative design record.
